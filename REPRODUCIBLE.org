...
: guix package -i boost --substitute-urls="http://penguin:8080"

Or use it with the guix-daemon using hydra as a fallback

: guix-daemon --build-users-group=guixbuild --substitute-urls="http://penguin:8080 http://mirror.guixsd.org http://hydra.gnu.org"

(for multiple substitutes to work make sure you are running Guix > 0.9)

To test the server by hand go to the URL

wget http://penguin:8080/nix-cache-info
