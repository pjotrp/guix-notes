...
not allowed to create this directory at build-time (FIXME: if it would
try it would be wrong because the build happens in an isolated
chroot).
...
The above example shows that it is possible to install software
outside /gnu/store directories (FIXME: but not possible from within a
package install). It should be stressed, however, that this should
only be done there is a good reason to do so. Inside /gnu/store a
...
FIXME: Small example of working configuration in /etc and /gnu/store
