...
allows for creating new Guix expressions. Alternatively create 
...
your purpose. Say we have the generic apache package, but we want to
configure it for one type of webserver: simply create the packager
...
result in different outcomes. The next evolutionary step, light-weight
containers in conjunction with GNU Guix, system administration can
avoid non-determinism altogether. Note that light-weight containers on
their own (such as Docker) are not enough to avoid non-determinism -
though they can be a part of the solution.
