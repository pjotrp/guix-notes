...
But the CWL workflow does not automatically fetch them. So, let's fix
that. We'll simply add them using IPFS (though, alternatively, we
could recreate them using 'bwa index' instead).
...
some other repo. See also https://github.com/genenetwork/guix-cw
