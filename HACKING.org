...
right LISP indendation.

Which creates a file 0001-gnu-patchname.patch and mail it with something like

: git send-email --from "Pjotr Prins <pjotr.guix@mymail.nl>" --to guix-patches@gnu.org 0001-gnu-patchname.patch

You may also need to install 'guix package -i git:send-email' to get
E-mail support.

Multiple patches can be passed in. Probably a good idea to try and
send the mail to yourself first.  Don't send the same E-mail twice ;).

To change credentials for the patch use [[https://help.github.com/articles/setting-your-email-in-git/][git config]].
...
: git send-email --from "Pjotr Prins <pjotr.guix@mymail.nl>" --to 25704@debbugs.gnu.org 0001-gnu-patchname.patch
...
The last command will compile and break at runtime.
