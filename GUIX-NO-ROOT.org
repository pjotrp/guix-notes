...
: proot -b /proc -b /dev -b /etc -b /bin -b /lib -b /lib64 -r . gnu/store/vir3lrwqy50pr8fkaf3m091dgbrja2n6-guix-0.13.0/bin/guix environment guix --ad-hoc help2man git strace pkg-config less vim binutils coreutils grep --no-grafts -- bash
: export PATH=/home/wrk/.guix-profile/bin
: cd guix
: ./bootstrap
...
Note that guix has been installed in /usr/local/bin this time (inside
proot). Now we need to run the new guix and daemon.

Restart the new daemon and install at leisure.
