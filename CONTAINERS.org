...
#+begin_src sh
./pre-inst-env guix pack --format=docker -S /bin=/bin conda bash coreutils
#+end_src sh

This builds an archive with both `conda` and `bash`. The package will be named
something like `/gnu/store/h7cm5p5m1jalirqwp5h5mb4cqxi7la4n-docker-pack.tar.gz`
...
coreutils has been added to provide the basic core commands
...
#+begin_src sh
cat /gnu/store/h7cm5p5m1jalirqwp5h5mb4cqxi7la4n-docker-pack.tar.gz | docker load
#+end_src sh
...
`profile:8klc3fipj4shwl1gdnc2w2makpkkp7kz`. This is the base image that will be
...
#+begin_src sh
docker image tag profile:8klc3fipj4shwl1gdnc2w2makpkkp7kz fredmanglis/guix-conda-base-img:latest
#+end_src sh
...
:NOTE: The above example isn't working at this time. It fails with:

#+begin_src sh
bash: /.conda/envs/default-env/bin/sambamba: No such file or directory
#+end_src sh

despite the fact that the file actually exists and is executable.

The `--volume sample:/data` portion of the command mount a directory named
`sample` on to the path `/data` in the container created by docker to run the
given command. This is to make sure the data you're working on is available to
